...
contract ChallengeContract is IChallengeCore, ERC721Pausable, AccessControl, Ownable {
    ...
    event CreateChallenge(address userAddress, uint256 tokenId, uint256 challengeId);

    mapping(uint256 => Challenge) internal challenges;
    mapping(uint256 => uint256) internal externalChallenges;
    ...
    function safeMintNFT(
        ...
        _challenge.amount = 1000000000;
        _challenge.tokenId = nextTokenId;
        _challenge.challengeId = challengeId;
        _challenge.createdAt = block.timestamp;
        _mint(_addr, nextTokenId);
        externalChallenges[challengeId] = nextTokenId;
        challenges[nextTokenId] = _challenge;
        emit CreateChallenge(_msgSender(), nextTokenId, challengeId);
    ...
    function getChallengeEr(uint256 _tokenId) public view returns (Challenge memory) {
        return challenges[_tokenId];
    }
    ...
    function getItemByExternal(uint256 _externalId)
        ...
        returns (Challenge memory)
    {
        uint256 tokenId = externalChallenges[_externalId];
